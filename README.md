...
1. Download and run `Codex.Tracker.exe` on Windows x64.
...
Release packages publish a self-contained `Codex.Tracker.exe` from version tags. Git-clone installations also use the repository updater and require the `main` branch with no local changes.
